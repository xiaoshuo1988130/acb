...
function renderHandoffPrompt(packet) {
  ...
  if (packet.fingerprint) {
    lines.push("", "## Workspace Fingerprint", "", renderWorkspaceFingerprint(packet.fingerprint));
  }
...
function renderBriefPrompt(packet) {
  ...
  if (packet.fingerprint) {
    lines.push(
      ...
      `- watch_files: ${packet.fingerprint.file_count || 0}`,
    );
  }
  if (packet.notes?.length) {
    lines.push("", "## Notes");
...
function renderWorkspaceFingerprint(fingerprint) {
  const lines = [
    `- algorithm: ${fingerprint.algorithm || "sha256"}`,
    `- watch_paths: ${(fingerprint.watch_paths || []).join(", ") || "none"}`,
    `- files: ${fingerprint.file_count || 0}`,
  ];
  const entries = Array.isArray(fingerprint.entries) ? fingerprint.entries.slice(0, 12) : [];
  if (entries.length) {
    lines.push("", "```text");
    for (const entry of entries) {
      lines.push(`${entry.type} ${entry.path}`);
    }
    if ((fingerprint.entries || []).length > entries.length) {
      lines.push(`... ${(fingerprint.entries || []).length - entries.length} more watched item(s) omitted`);
    }
    lines.push("```");
  }
  return lines.join("\n");
