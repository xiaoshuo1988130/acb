...
function readStore() {
  ...
  try {
    ...
    const version = parsed.version == null ? STORE_VERSION : Number(parsed.version);
    if (!Number.isInteger(version) || version < 1) {
      return { ok: false, error: `ACB store has an invalid version: ${parsed.version}` };
    }
    if (version > STORE_VERSION) {
      return { ok: false, error: `ACB store version ${version} is newer than supported version ${STORE_VERSION}. Upgrade ACB or restore a backup.` };
    }
    return { ok: true, store: { version: STORE_VERSION, packets: parsed.packets } };
  } catch (error) {
...
function writeStore(store) {
  ...
  const normalizedStore = {
    ...
    packets: Array.isArray(store?.packets) ? store.packets : [],
  };
  ...
  try {
    ...
    fs.writeFileSync(fd, `${JSON.stringify(normalizedStore, null, 2)}\n`);
    fs.fsyncSync(fd);
    fs.closeSync(fd);
    fd = null;
