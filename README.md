...
Expose it as a local stdio MCP server. An upstream agent can call `save_handoff`, then the next agent can call `read_latest_handoff` or `search_handoffs` for the current workspace.
